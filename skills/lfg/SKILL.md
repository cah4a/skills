...
The bar lives in the **coder** skill (`~/.claude/skills/coder/SKILL.md`) — one file, two uses. Every brief carries its absolute path, so the subagent writes to the bar; you read it once at the start, so you judge by the same bar. Subagents will hand you code that compiles, passes the test they wrote, and quietly rots the codebase. Your job is to not let that through: read every diff the way a careful senior engineer reads a PR they'll have to maintain.
...
2. **Dispatch** — brief a fresh subagent: the step's goal, its slice of the map, the path to the coder skill, nothing speculative. The coder skill makes it report back its diff *and* what it noticed — a subagent briefed this way is a subagent you can judge.
3. **Judge** — read the diff against the coder bar, then run the build or tests to prove the change *runs*. That's all tests prove; *you* decide if it's *good*. Below the bar → back to a fresh subagent with specific notes, same step. Above it → fold what it learned into the map.
